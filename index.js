...
$('#randomQuestion').text(question(items))

$('#question').on('click',"#removeButton",function() {
    $(this).parents('div.response').remove('div.response');
});

$('#submitButton').on('click',function() {
      let randomColor = Math.floor(Math.random()*16777215).toString(16);
      let random =  "#"+ randomColor;
      let displayName_txt = $('#displayName').val();    
      let response_txt = $('#reply').val();
  $('#question').append('<div class="response"><div id="imgid" style="background-color: '+random+'"><img src="Avatar icon.png" alt="avatar"></div><div class="subContainer"><div class="answer">'+response_txt+'</div><div id="userName"><div><h3>'+displayName_txt+'</h3></div><form id="buttonControl"><input id="removeButton" type="button" value="Remove"><input id="editButton" type="button" value="Edit"></form></div></div></div>');
});
$('#question').on('click',"#editButton",function() {
  let child = $(this).parents()[2];
  console.log(child);
  let edit = $(child).children()[0];
  console.log(edit);
  let newResponse_txt = prompt("Edit What you would like to change", $(edit).text());
  $(edit).text(newResponse_txt);                                
});
